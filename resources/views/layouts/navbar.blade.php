<nav class="bg-white shadow-md sm:hidden">
    <div class="flex flex-row justify-between items-center">
        <div class="px-4 py-6 text-center border-b">
            <h1 class="text-xl font-bold leading-none"><span class="text-yellow-700">Absen</span> Karyawan</h1>
        </div>
        <button class="mx-2 hover:bg-slate-300 p-2 rounded" id="linksButton">
        ...
        <ul class="w-full">
            <li>
                ...
                <a href="/dashboard/cuti-sakit"
                    class="flex justify-center items-center {{ Request::is('dashboard/cuti-sakit') ? 'bg-yellow-200' : '' }} font-bold text-sm text-yellow-900 py-3 px-4">
                    Ajukan Cuti / Sakit
                </a>
                @can('admin')
                    <p class="flex justify-center items-center text-sm text-gray-400 py-3 px-4 mt-3">Administration</p>
                    <a href="/dashboard/pulang-awal"
                        class="flex justify-center items-center {{ Request::is('dashboard/pulang-awal') ? 'bg-yellow-200' : '' }} rounded-xl font-bold text-sm text-yellow-900 py-3 px-4">
                        Pengajuan Pulang Awal
                    </a>
                    <a href="/dashboard/pengajuan-cuti-sakit"
                        class="flex justify-center items-center {{ Request::is('dashboard/pengajuan-cuti-sakit') ? 'bg-yellow-200' : '' }} rounded-xl font-bold text-sm text-yellow-900 py-3 px-4">
                        Daftar Cuti / Sakit
                    </a>
                    <a href="/dashboard/riwayat-absen"
                        class="flex justify-center items-center {{ Request::is('dashboard/riwayat-absen') || Request::is('dashboard/riwayat-absen/filter') ? 'bg-yellow-200' : '' }} rounded-xl font-bold text-sm text-yellow-900 py-3 px-4">
                        Riwayat Absen
                    </a>
                    <a href="/dashboard/daftar-karyawan"
                        class="flex justify-center items-center {{ Request::is('dashboard/daftar-karyawan') || Request::is('dashboard/register') ? 'bg-yellow-200' : '' }} rounded-xl font-bold text-sm text-yellow-900 py-3 px-4">
                        Daftar Karyawan
                    </a>
                @endcan
                <p class="flex justify-center items-center text-sm text-gray-400 py-3 px-4 mt-3">User</p>
                <a href="/dashboard/profile"
                    class="flex justify-center items-center rounded-xl font-bold text-sm text-yellow-900 py-3 px-4">
                    <svg
...
<aside class="hidden sm:block fixed inset-y-0 left-0 bg-white shadow-md max-h-screen w-60">
